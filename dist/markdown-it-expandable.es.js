...
function skipBulletListMarker(state, startLine) {
  ...
  const max = state.eMarks[startLine];
  const marker2 = state.src.charCodeAt(pos++);
...
const classyList = (state, startLine, endLine, silent) => {
  let start = state.bMarks[startLine] + state.tShift[startLine];
  ...
  let listLines;
  ulOlToken.map = listLines = [startLine, 0];
  ...
  while (nextLine < endLine) {
    ...
    liToken = isExpandable.test(currentLineItem) ? state.push("list_item_open", `li class="lvl-${indent} expandable"`, 1) : state.push("list_item_open", `li class="lvl-${indent}"`, 1);
    liToken.markup = String.fromCharCode(markerCharCode);
    let itemLines;
    liToken.map = itemLines = [startLine, 0];
    if (isOrdered) {
...
export { classyList, collapsiblePlugin as default };
